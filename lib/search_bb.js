...
/**
 * Bitboard-native search (baseline + tiny TT):
 * - Packed raw moves
 * - Make/Unmake
 * - Alpha-Beta (no qsearch, no null-move)
 * - Simple MVV-LVA ordering + killers/history
 * - Material evaluation only
 * - Transposition Table (Map) with Zobrist
 *   • counts: prunes (beta cutoffs), ttHits, ttStores
 */
...
// Ensure board.bb exists and is *fresh* using the engine's own builder.
// No guessing, no custom mapping.
function ensureBoardBB(board) {
    if (BB.syncFromBoard && typeof BB.syncFromBoard === 'function') {
        BB.syncFromBoard(board);         // preferred: rebuild from current board
        return;
    }
    ...
    if (BB.attachToBoard && typeof BB.attachToBoard === 'function') {
        board.bb = BB.attachToBoard(board);
        return;
    }
    throw new Error('Bitboard init not available; remove custom fallback mapping.');
}
...
function bbStateFromBoard(board) {
    // Always rebuild bitboards from the board snapshot using engine helpers.
    ensureBoardBB(board);

    // Resolve side to move robustly (strings 'w'/'b' or enum).
    const stmResolved =
        (board.turnColor === 'w' || board.turnColor === 'W') ? Color.WHITE :
            (board.turnColor === 'b' || board.turnColor === 'B') ? Color.BLACK :
                board.turnColor; // assume already Color.WHITE/Color.BLACK
    ...
    // Recompute OCC on the clone to be extra-safe.
    recomputeOcc(S.bb);
...
function orderMoves(S, arr, ply) {
    const side = S.stm;
    const ttBest = ttBestMove(S);
    const scored = new Array(arr.length);
    for (let i = 0; i < arr.length; i++) {
        const m = arr[i];
        let s = 0;
        if (m === ttBest) {
            s = 2_000_000; // TT best at front
        } else if (M_cap(m)) {
...
let _nodes = 0;
let _cutoffs = 0;     // beta cutoffs (counted as "nodes pruned")
let _ttHits = 0;
let _ttStores = 0;

const TT = new Map(); // key: S.hash (BigInt) -> {depth, score, flag, bestMove}
...
function alphaBeta(S, depth, alpha, beta, ply) {
    ...
    // TT probe
    const probe = ttProbe(S.hash, depth, alpha, beta);
    ...
    if (depth <= 0) {
        ...
        const v = sign * materialEval(S);
        ttStore(S.hash, depth, v, TT_EXACT, 0);
        return v;
    }
    ...
    if (_legalMoves.length === 0) {
        ...
        const v = inCheck(S, S.stm) ? (-INF + 1) : 0;
        ttStore(S.hash, depth, v, TT_EXACT, 0);
        return v;
    }
        ...
        if (score > best) {
            ...
            if (score > alpha) {
                ...
                if (alpha >= beta) {
                    ...
                    _cutoffs++;
                    ttStore(S.hash, depth, best, TT_BETA, m);
                    return best;
                }
            }
        }
...
function searchRoot(board, depth) {
    ...
    const n0 = _nodes, h0 = _ttHits, s0 = _ttStores, c0 = _cutoffs;

    const rootMoves = genLegalMovesArray(S);
    if (rootMoves.length === 0) {
        return { best: 0, score: 0, nodes: 0, ttHits: 0, ttStores: 0, prunes: 0 };
    }
...
function searchIter(board, maxDepth) {
    TT.clear();
    _nodes = 0; _ttHits = 0; _ttStores = 0; _cutoffs = 0;

    let bestMove = 0, lastScore = 0;
    let nodes = 0, ttHits = 0, ttStores = 0, prunes = 0;
    ...
    for (let d = 1; d <= maxDepth; d++) {
        ...
        nodes   += s.nodes   | 0;
        ttHits  += s.ttHits  | 0;
        ttStores+= s.ttStores| 0;
        prunes  += s.prunes  | 0;
    }
    return { best: bestMove | 0, score: lastScore | 0, nodes, ttHits, ttStores, prunes };
}
...
function rootMoveToSAN(board, m) {
    ...
    // Make sure occupancy used for disambiguation is fresh.
    if (BB.syncFromBoard && typeof BB.syncFromBoard === 'function') {
        BB.syncFromBoard(board);
    } else if (!board.bb || !board.bb.OCC) {
        // fallback to init if sync isn’t present
        if (BB.initFromBoard) board.bb = BB.initFromBoard(board);
    }

    const from = M_from(m), to = M_to(m);
    const fromX = sqX(from), fromY = sqY(from);
    const toX   = sqX(to),   toY   = sqY(to);

    const piece = board.get(fromX, fromY);
    if (!piece) return '??';

    const pieceType = piece.toFEN().toUpperCase();
    const dest = board.xToFile[toX] + board.yToRank[toY];

    let target = board.get(toX, toY);
    let isCap = !!(target && target.color !== piece.color);

    if (!isCap && pieceType === 'P' && toX !== fromX && !target) {
        const adj = board.get(toX, fromY);
        if (adj && (adj.toFEN().toUpperCase() === 'P') && adj.color !== piece.color && adj.enPassantable) {
            isCap = true;
        }
    }
    ...
    const isPromo = (pieceType === 'P') && (toY === 0 || toY === 7);
    const promoStr = isPromo ? '=Q' : '';

    if (pieceType === 'P') {
        if (isCap) return board.xToFile[fromX] + 'x' + dest + promoStr;
        return dest + promoStr;
    }

    const occ = board.bb.OCC.ALL; // now guaranteed fresh
    ...
    const toMask = BB.BB_SQ((toY << 3) | toX);
    let ambig = false, sameFile = false, sameRank = false;
    for (const other of team) {
        if (attacksMask(other) & toMask) {
            ambig = true;
            if (other.x === fromX) sameFile = true;
            if (other.y === fromY) sameRank = true;
        }
    }

    let dis = '';
    const fileChar = board.xToFile[fromX];
    const rankChar = board.yToRank[fromY];
    if (ambig) {
        if (sameFile && sameRank) dis = fileChar + rankChar;
        else if (sameFile) dis = rankChar;
        else dis = fileChar;
    }

    return pieceType + dis + (isCap ? 'x' : '') + dest;
}
