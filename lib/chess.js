...
module.exports = class Chess {
    ...
    push(move) {
        ...
        if (typeof move === 'string') {
            ...
            const built = this.board.buildMove(clean, currColor);
    ...
    legalMovesAI(opts = {}) {
        ...
        for (const piece of team) {
            ...
            for (const [dx, dy] of piece.moveSet) {
                ...
                if (isPromotion) {
                    out.push({ piece, destX: x, destY: y, promo: 'Q', isCapture: !!isCapture, isPromotion: true });
                    out.push({ piece, destX: x, destY: y, promo: 'N', isCapture: !!isCapture, isPromotion: true });
                } else {
                    out.push({ piece, destX: x, destY: y, promo: null,  isCapture: !!isCapture, isPromotion: false });
                }
            }
        }

        // PSEUDO castling: require rook present, both unmoved, path clear.
        // (Do NOT test attacked squares here; the search will filter after push.)
        if (!capturesPromosOnly) {
            ...
            if (king && king.firstMove) {
                ...
                if (rookK instanceof Rook && rookK.firstMove && this.board.pathClear(king.x, y, 7, y)) {
                    out.push({ piece: king, destX: king.x + 2, destY: y, promo: null, isCapture: false, isPromotion: false, isCastle: true });
                }
    ...
    // Helper: use buildMove to validate if pseudo legal move is legal
    validate(entry) {
        // Uses your existing builder that understands castling + EP + promotions
        return this.board.buildMoveFromCoords(entry.piece, entry.destX, entry.destY, entry.promo || null);
    }
