...
module.exports = class Chess {
    ...
    undo() {
        ...
        if (prevState.currentState) {
            this.states.set(prevState.currentState, this.states.get(prevState.currentState) - 1);
        }
    ...
    move(moveStr) {
        ...
        // generate fen string for resulting position
        let fen = this.board.toFEN();
        let hash = this.board.stateHash(fen);
        this.fen = fen;

        // let hash = this.board.key;
        // this.key = hash;

        // record game state, track stalemate conditions
        if (moveStr.includes("x") || moveStr[0] === moveStr[0].toLowerCase()) {
            this.history.push(
                new GameState(this.check, this.prevMove, this.fiftyMoveCounter, this.states, null)
            );
            this.states = new Map();
            this.fiftyMoveCounter = 0;
        } else {
            this.history.push(
                new GameState(this.check, this.prevMove, this.fiftyMoveCounter, null, hash)
            );
    ...
    legalMoves(piece) {
        ...
        pieces.forEach((piece) => {
            piece.moveSet.forEach((move) => {
                ...
                if (
                    ![Status.INVALIDMOVE, Status.STILLINCHECK, Status.PUTSINCHECK].includes(status)
                ) {
        ...
        if ((!piece || piece instanceof King) && !this.check) {
            ...
            if (!(castleFEN[0] === "-")) {
                ...
                castleMoves.forEach((castle) => {
                    ...
                    if (
                        ![Status.INVALIDMOVE, Status.STILLINCHECK, Status.PUTSINCHECK].includes(
                            status
                        )
                    ) {
                        moveSet.push(castle);
    ...
    sortMoves(moves) {
        for (let i = 0; i < moves.length; i++) {
            if (moves[i].includes("x") || moves[i].includes("=") || moves[i].includes("O")) {
