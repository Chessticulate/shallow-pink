...
module.exports = class Chess {
    ...
    generateMoveStrs(piece, x, y) {
        ...
        let origFile = addressing.xToFile[piece.x];
        let origRank = addressing.yToRank[piece.y];
        let destFile = addressing.xToFile[x];
        let destRank = addressing.yToRank[y];
    ...
    legalMoves() {
        ...
        pieces.forEach(piece => {
            piece.moveSet.forEach(move => {
                ...
                let status = this.move(moveStrs[0]);
    ...
    toFEN() {
        ...
        if (enPassantPiece) {
            let enPassfile = addressing.xToFile[enPassantPiece.x];
