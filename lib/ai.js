...
function qsearch(alpha, beta, move) {
    ...
    let best;
    const stm = (CHESS.turn % 2) ? Color.WHITE : Color.BLACK;
    ...
    for (const entry of entries) {
        const built = CHESS.validate(entry); // -> [moves, orig, dest, meta] or null
        if (!built) continue;
...
function miniMaxRec(depth, alpha, beta, move) {
    ...
    if (depth === 0) {
        const qs = qsearch(alpha, beta, move);
        const tt = CHESS.table.get(CHESS.board.hash);
        if (tt !== undefined) { data.tptNodes++; return tt.score; }
        CHESS.table.set(CHESS.board.hash, new TableEntry(move, qs));
        return qs;
    }
