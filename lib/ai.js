...
function miniMaxRec(chess, depth, alpha, beta, evaluatedPositions) {
    ...
    if (depth === 0 || chess.gameOver) {
        // this node memoization should be replaced with a transposition table
        if (chess.transpositionTable.get(fen) !== undefined) {
            // data.memoizedNodes++;
            return chess.transpositionTable.get(fen);
        }
        let score = evaluate(chess);
        chess.transpositionTable.set(fen, score);
        return score;
    }
...
function miniMax(chess, depth) {
    ...
    for (let i = 0; i < moves.length; i++) {
        chess.move(moves[i]);
        let score = miniMaxRec(chess, depth-1, alpha, beta);
