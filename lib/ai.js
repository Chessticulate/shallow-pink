...
function miniMaxRec(chess, depth, alpha, beta, move) {
    ...
    if (depth === 0 || chess.gameOver) {
        let storedFen = chess.transpositionTable.get(fen);
        if (storedFen !== undefined) {
            // data.memoizedNodes++;
            return storedFen.score;
        }
        let score = evaluate(chess);
        chess.transpositionTable.set(fen, new TableEntry(move, score));
        return score;
    }
...
function miniMax(chess, depth) {
    ...
    for (let i = 0; i < moves.length; i++) {
        ...
        // console.log(storedFen);

        if (storedFen !== undefined) {
            moveMap.set(storedFen.move);
            score = storedFen.score;
            console.log('stored FEN move ', storedFen.move);
            console.log('stored FEN score', storedFen.score);
        }
