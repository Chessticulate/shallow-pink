...
function orderEntries(entries, ply = 0) {
    ...
    function features(e) {
        ...
        if ('isCapture' in e || 'isPromotion' in e) {
            ...
            if (e.isCapture && Number.isInteger(e.destX) && Number.isInteger(e.destY)) {
                const victim = CHESS.board.get(e.destX, e.destY) || CHESS.board.get(e.destX, e.piece?.y);
                try { victimScore = Math.abs(victim?.value?.('mg') ?? 0); } catch {}
            }
            return {
    ...
    entries.sort((a, b) => {
        ...
        const baseA = (A.isPromotion ? 2 : 0) + (A.isCapture ? 1 : 0);
        const baseB = (B.isPromotion ? 2 : 0) + (B.isCapture ? 1 : 0);
        if (baseA !== baseB) return baseB - baseA; // promos > captures > quiets

        // Killer bonus only for quiet moves
        const killerA = (!A.isCapture && !A.isPromotion && isKiller(ply, a)) ? 1000 : 0;
        const killerB = (!B.isCapture && !B.isPromotion && isKiller(ply, b)) ? 1000 : 0;
...
function miniMaxRec(depth, alpha, beta, move, ply = 0) {
    ...
    if (depth === 0) return qsearch(alpha, beta, move); // leave qsearch as-is
    ...
    for (const entry of entries) {
        ...
        // Beta cutoff → record killer if this move was QUIET (not capture, not promotion)
        if (beta <= alpha) {
