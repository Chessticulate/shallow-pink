...
function miniMaxRec(chess, depth, depth2, alpha, beta) {
    ...
    let moves = legalMoves(chess.toFEN());
    ...
    for (let i = 0; i < moves.length; i++) {
        ...
        let score = miniMaxRec(chess, depth-1, depth2+1, alpha, beta);
        chess.undo();
...
function miniMax(fen, depth) {
    ...
    moves.forEach(move => {
        ...
        chess.move(move);
        console.log(" ", evaluate(chess.toFEN()));
        let score = miniMaxRec(chess, depth-1, 1, alpha, beta);
        // prev state is null, which is causing this undo to return before doing anything.
        chess.undo();
