...
function normalizeTurnColor(tc, turn) {
    if (tc === Color.WHITE || tc === 'W' || tc === 'w') return 'w';
    if (tc === Color.BLACK || tc === 'B' || tc === 'b') return 'b';
    if (typeof tc === 'string') {
        const s = tc.toLowerCase();
        if (s === 'white') return 'w';
        if (s === 'black') return 'b';
    }
    // fallback using move count parity if needed
    if (typeof turn === 'number') return (turn % 2 === 0 ? 'w' : 'b'); // adjust if your engine defines parity differently
    return 'w';
}

// replace your miniMaxBB with this version
function miniMaxBB(CHESS, depth) {
    // *** important: keep board.turnColor in sync for search_bb ***
    CHESS.board.turnColor = normalizeTurnColor(CHESS.turnColor, CHESS.turn);
    ...
    const san = rootMoveToSAN(CHESS.board, best);
