...
module.exports = class Board {
    ...
    move(moveList, undoing, temp) {
        // don't need to record moves if we are undoing
        if (!undoing) {
            // temp == true => this is being called from piece.canMove()
            if (temp) {
                this.tempMove = [];
            }
            // otherwise, this is being called from chess.move()
            else {
                this.prevMove = [];
            }
        }
        ...
        moveList.forEach(moveObj => {
            ...
            */
            ...
            if (temp) {
                this.tempMove.unshift(new Move(piece, origX, origY, firstMove));
            }
            else {
        ...
        if (undoing) {
            ...
            if (this.enPassantList.length > 3) {
                this.enPassantList.shift();
            } 
        }
    }
    ...
    undo(temp) {
        if (temp) {
            this.move(this.tempMove, true, true);
            this.tempMove = null;
        }
        else {
            this.move(this.prevMove, true, false);
            this.prevMove = null;
        }
    }
