...
module.exports = class Board {
    ...
    move(piece, x, y) {
        ...
        let pieceTaken = this.getByAddress(x, y);
        if (piece instanceof Pawn && pieceTaken === null) {
            let adjacent = this.getByAddress(x, piece.y);
            if (adjacent instanceof Pawn && adjacent.enPassantable) {
                pieceTaken = adjacent;
                this.set(adjacent.x, adjacent.y, null);
            }
        }
    ...
    castle(address, color) {
        const king = this.getById('king', color);
        const rook = this.getById(`rook${address.toUpperCase()}`, color);
        ...
        // king side
        if (address.toLowerCase() === 'k') {

            // all squares between king and rook need to be null
            if (this.board[king.y][king.x+1] !== null || this.board[king.y][king.x+2] !== null) {
                return false;
            }

            // all squares between king and rook cannot be in check
            this.set(king.x, king.y, null);
            this.set(king.x+1, king.y, king);
            king.x = king.x+1;
            if (this.checkForCheck(color) === true) {
                this.set(king.x, king.y, null);
                this.set(initX, king.y, king);
                king.x = initX;
                return false;
            }
            ...
            // perform castle, king can stay where it is from previous checkForCheck
            this.set(rook.x, rook.y, null);
            this.set(rook.x-2, rook.y, rook);
            rook.x = rook.x-2;
        ...
        // queen side
        if (address.toLowerCase() === 'q') {

            // all squares between king and rook are null
            if (this.board[king.y][king.x-1] !== null || 
                this.board[king.y][king.x-2] !== null ||
                this.board[king.y][king.x-3] !== null) {
                return false;
            }
            ...
            // perform castle, king is already set from previous checkForCheck
            this.set(rook.x, rook.y, null);
            this.set(rook.x+3, rook.y, rook);
            rook.x = rook.x+3;
