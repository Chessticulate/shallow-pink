...
const {fileToX, rankToY} = require("./addressMaps");
...
module.exports = class Board {
    constructor (fenStr) {
        ...
        boardState.forEach(char => {
            ...
            switch(char.toUpperCase()) {
                ...
                case 'P':
                    ...
                    this.board.teamMap[color].push(this.board.set(x, y, pawn));
                    break;
                case 'N':
                    this.board.teamMap[color].push(this.board.set(x, y, new Knight(color, x, y)));
                    break;
                case 'B':
                    this.board.teamMap[color].push(this.board.set(x, y, new Bishop(color, x, y)));
                    break;
                case 'R':
                    this.board.teamMap[color].push(this.board.set(x, y, new Rook(color, x, y)));
                    break;
                case 'Q':
                    this.board.teamMap[color].push(this.board.set(x, y, new Queen(color, x, y)));
                    break;
                case 'K':
                    let king = new King(color, x, y);
                    this.board.teamMap[color].push(this.board.set(x, y, king));
                    if (color === Color.WHITE) {
                        this.board.whiteKing = king;
                    }
                    else this.board.blackKing = king;
                    break;
                default:
                    x += parseInt(char);
            }
        ...
        if (enPassant !== "-") {
            ...
            py += py === 6 ? 1 : -1;
            this.enPassantable = this.get(px, py);
        }
    }
    ...
    move(moveList, undoing) {
        ...
        if (undoing) {
            ...
            this.enPassantable = this.prevEnPassantable;
            if (this.enPassantable) {
                this.enPassantable.enPassantable = true;
            }
            this.prevEnPassantable = null;
        }
        else {
            this.prevEnPassantable = this.enPassantable;
            if (this.prevEnPassantable) {
                this.prevEnPassantable.enPassantable = false;
            }
